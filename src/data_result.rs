...
impl<R> DataResult<R> {
    ...
    pub fn error(message: impl Error + 'static) -> Self {
    ...
    pub fn error_with_lifecycle(message: impl Error + 'static, lifecycle: Lifecycle) -> Self {
        Self::new(
            DataResultKind::Error {
                partial: None,
                error: DataError::new(ErrorMessage::new(message)),
            },
            lifecycle,
        )
    }

    /// Creates a new *failed* data result with the provided error (stored as a `String`) and the default lifecycle.
    pub fn error_string(message: impl Into<Box<str>>) -> Self {
        Self::error_string_with_lifecycle(message, Lifecycle::Experimental)
    }

    /// Creates a new *failed* data result with the provided error (stored as a `String`) and lifecycle.
    pub fn error_string_with_lifecycle(message: impl Into<Box<str>>, lifecycle: Lifecycle) -> Self {
        Self::new(
            DataResultKind::Error {
                partial: None,
                error: DataError::new(ErrorMessage::string(message)),
            },
            lifecycle,
        )
    }
    ...
    pub fn partial(value: R, message: impl Error + 'static) -> Self {
    ...
    /// Creates a new *partial* data result with the provided value, error (stored as a `String`) and the default lifecycle.
    pub fn partial_string(value: R, message: impl Into<Box<str>>) -> Self {
        Self::partial_string_with_lifecycle(value, message, Lifecycle::Experimental)
    }

    /// Creates a new *partial* data result with the provided value, error (stored as a `String`) and lifecycle.
    pub fn partial_string_with_lifecycle(
        value: R,
    ...
    pub fn extract(self) -> (Option<R>, DataResult<()>) {
        match self.kind {
            DataResultKind::Success(r) => (Some(r),
            ...
            DataResultKind::Error { partial, error } => {
                ...
                (partial, DataResult::new(DataResultKind::Error {
                    partial: new_partial,
                    error
                }, self.lifecycle))
            }
    ...
    pub fn status(self) -> DataResultStatus<R> {
        match self {
            Self {kind: DataResultKind::Success(r), ..} => DataResultStatus::Success(r),
            Self {kind: DataResultKind::Error { partial: Some(r), .. }, ..} => DataResultStatus::Partial(r),
            Self {kind: DataResultKind::Error { partial: None, .. }, ..} => DataResultStatus::Failed,
        }
    }
...
impl ErrorMessage {
    ...
    pub fn new(display: impl Error + 'static) -> ErrorMessage {
        Self::Dynamic(Box::new(display))
    }

    /// Creates a new *string* error message.
    pub fn string(string: impl Into<Box<str>>) -> Self {
        Self::String(string.into())
    }

    /// Creates a new *additional* error message.
    pub fn additional(string: impl Into<Box<str>>) -> Self {
