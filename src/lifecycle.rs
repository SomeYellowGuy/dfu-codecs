...
/// A marker to convey the lifecycle of some object. It could be stable, experimental or deprecated.
...
impl Lifecycle {
    ...
    /// Adds all lifecycles, returning the resultant lifecycle which matches the most restrictive of them.
    ///
    /// This function returns the new lifecycle, wrapped in a [`Some`], if `lifecycles` is not empty.
    /// Otherwise, a [`None`] is returned.
    pub fn add_all(lifecycles: impl IntoIterator<Item = Self>) -> Option<Self> {
        lifecycles.into_iter().reduce(|a, b| a + b)
    }
}
